...
DROP USER suser1 CASCADE; -- 기존 사용자 삭제(현재 접속되어 있으면 삭제 안 됨)
	-- CASCADE option : 관련 스키마 개체들도 함께 삭제.  Default는 No Action
CREATE USER suser1 IDENTIFIED BY 1234  -- 사용자 ID : suser, 비밀번호 : 1234
    DEFAULT TABLESPACE USERS
    TEMPORARY TABLESPACE TEMP;
GRANT connect, resource, dba TO suser1; -- 권한 부여
...
CREATE TABLE 통합스케줄(
    ...
    팀_번호 NUMBER(5),
    통합스케줄_이름 VARCHAR(20),
    날짜 DATE,    -- 통합 스케줄은 항상 비고정으로 취급(일회성)
    시작시간 NUMBER(3),
    종료시간 NUMBER(3),
    메모 VARCHAR(50),
    FOREIGN KEY (팀_번호) REFERENCES 팀(팀_번호)
...
-- 통합스케줄(통합_번호, 팀_번호, 통합스케줄_이름, 날짜, 시작시간, 종료시간, 메모)
INSERT INTO 통합스케줄 VALUES(1, 1, '회의1', '2022/11/15', 17, 18, '첫번째 회의');
